...
def arena_fish_tracking(aviFile, ROI, startFrame=0, maxNumFrames=432000, FPS=120, sepFigurePath=None, sepMoviePath=None, plot=True, cropOp=True, saveCroppedMovie=True, display=False, trackTail=True, larvae=True, inPlace=True, sepPlace=False):
    ...
    if larvae:
        ...
        areaLimit=2000
        cropSize=[256,256]
        BGstartFrame=(3*60)*FPS # Juveniles can freeze for longer than larvae. Frame of movie with which to start computing the background... avoids having fish in background if frozen for first few minutes after transfer
    #    l=0
...
#        print(f)
        ...
        if f==0 and saveCroppedMovie:
            if inPlace:
                print('Creating cropped Movie in raw data folder')
                movieFolder=d + '\\Movies\\'
                AZU.cycleMkDir(movieFolder)
                croppedSegPath=movieFolder + expName + '_tail_segmented_cropped.avi'
                croppedTailSegMovOutinPlace = cv2.VideoWriter(croppedSegPath, cv2.VideoWriter_fourcc(*'DIVX'), FPS, (cropSize[1],cropSize[0]))
                croppedSegPath=movieFolder + expName + '_cropped.avi'
                croppedMovOutinPlace = cv2.VideoWriter(croppedSegPath, cv2.VideoWriter_fourcc(*'DIVX'), FPS, (cropSize[1],cropSize[0]))
            
            if sepPlace:
                print('Creating cropped Movie in new analysis folder')
                croppedSegPath=sepMoviePath + expName + '_tail_segmented_cropped.avi'
                croppedTailSegMovOutsepPlace = cv2.VideoWriter(croppedSegPath, cv2.VideoWriter_fourcc(*'DIVX'), FPS, (cropSize[1],cropSize[0]))
                croppedSegPath=sepMoviePath + expName + '_cropped.avi'
                croppedMovOutsepPlace = cv2.VideoWriter(croppedSegPath, cv2.VideoWriter_fourcc(*'DIVX'), FPS, (cropSize[1],cropSize[0]))
        ...
        tailThreshold = threshold_level*1.2
        # Threshold            
        ...
        if len(contours) == 0:
            ...
            noContoursCount=0   # Reset lost contour count if we found the fish again
...
# -------------Tail Tracking ---------------------------------------------------------------------------
                if trackTail:
                    ...
                    if f==0:
                        ...
                        flag=True
                        print('threshold = ' + str(threshold_level))
                        print('tail threshold = ' + str(tailThreshold))
                        while flag and f==0: # find length of tail if on first frame
                            ...
                            print('Tail max = ' + str(np.max(arcVals_sm)))
                ...
                write=cv2.cvtColor(current, cv2.COLOR_GRAY2BGR)
                resFullNoMarker=write[startIdY:endIdY,startIdX:endIdX]
#                if display:
#                    res=cv2.resize(resFull, (0, 0), fx=2, fy=2) 
#                    cv2.imshow("Display", res)
                if f>0 and saveCroppedMovie:
                    if inPlace:
                        croppedTailSegMovOutinPlace.write(resFull)
                        croppedMovOutinPlace.write(resFullNoMarker)
                    if sepPlace:
                        croppedTailSegMovOutsepPlace.write(resFull)
                        croppedMovOutsepPlace.write(resFullNoMarker)
...
# Save Tracking Summary
        if inPlace: 
            trackingFiguresDir=d + 'TrackingFigures\\'
            AZU.cycleMkDir(trackingFiguresDir)
         
            if(f == 0):
                path=trackingFiguresDir + expName + '_initial_tracking.png'
                plt.savefig(path, dpi=300)
            
                plt.figure('backgrounds')
                plt.imshow(backgroundFull)
                path=trackingFiguresDir + expName +'_initial_background.png'
                plt.savefig(path, dpi=300)
                plt.close('backgrounds')
       
            if(f == numFrames-1):
                path=trackingFiguresDir + expName +'_final_tracking.png'
                plt.savefig(path, dpi=300)
                plt.figure('backgrounds')
                plt.imshow(backgroundFull)
                path=trackingFiguresDir + expName +'_final_background.png'
                plt.savefig(path, dpi=300)
                plt.close('backgrounds')
        
        if sepPlace:
            trackingFiguresDir=sepFigurePath
            AZU.cycleMkDirr(trackingFiguresDir)
...
# Close Video Files
    if saveCroppedMovie:
        croppedTailSegMovOutsepPlace.release()
        croppedMovOutsepPlace.release()
    vid.release()
    print('Finished tracking')
   
    # Return tracking data
    return fxS, fyS, bxS, byS, exS, eyS, x_seg_data,y_seg_data,areaS, ortS, motS
...
def createSingleFrameVid(aviFile,path,expName,frame=5, FPS=120):
    '''
    Creates a single framed video from aviFile and saves it at given path under 
    a given file name. This file can be used to project the background of the 
    movie for figures etc and provides a convenient place to access all 
    analysed movies; for ROI defining for example, or plotting tracking data.

    Parameters
    ----------
    aviFile : String
        Full path to the raw movie
    path : String
        Output path where single frame will be saved
    expName : String
        Name of the experiment inferred from the avi filename
    frame : int, optional
        Which frame of the movie to save. The default is 5.
    FPS : int, optional
        Framerate of the movie in Hz. The default is 120.

    Returns
    -------
    saveName : String
        Full path of the saved, single frame movie.
    ...
    ## grab frame to use as a template for the ROIs
    im=AZU.grabFrame(aviFile,frame)
    saveName=path+expName + '_template.avi'
